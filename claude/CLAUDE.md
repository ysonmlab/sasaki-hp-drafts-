...
- **画像ファイルの扱い**: レイアウト確認・alt文言検討など内容確認が必要な場合を除き、画像ファイルをReadツールで開かず、ファイルパスの参照・リネーム・移動などパス操作のみで済ませること（トークン消費削減のため）。
...
- **多言語ページの同期**: `{page-name}.html`（日本語版）の文言・内容を修正した場合は、対応する `{page-name}-en.html`（英語版）にも同じ修正を反映すること。修正対象が住所・固有名詞・リンク先など日英で共通の情報の場合は特に漏らさないこと。英語版側の訳文・表現は既存の対応ページの言い回しに合わせる。
